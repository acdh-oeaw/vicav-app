...
describe('VICAV Compare samples window', function() {
			...
			cy.contains('Samples').click().then(() => {
				cy.contains('Compare Locations').click().then(() => {
				    ...
				    cy.get('[data-snippetID=compare-samples-result]').then((el) => {
				    	cy.url().should('contain', '[crossSamplesResult,location|Tunis2+age|0%2C100+person|Test1+sentences|1+word|nh%C4%81%E1%B9%9B+sex|,open]')	
	...
	it('should only show the given sentences ', () => {
		cy.visit('http://localhost:8984/vicav/#map=[biblMarkers,_samples_,]&1=[crossSamplesResult,location|Tunis2+age|0%2C100+person|+sentences|2+word|,open]')
	...
	it ('supports wildcards and highlights in word search', () => {
		cy.visit('http://localhost:8984/vicav/#map=[biblMarkers,_samples_,]&1=[crossSamplesResult,location|Tunis2%2CTest+age|+person|+sentences|+word|b%2Ain%C5%BE%2A')
	...
	it ('supports search by gender', () => {
		cy.visit('http://localhost:8984/vicav/#map=[biblMarkers,_samples_,]&1=[crossSamplesResult,location|Test+age|+person|+sentences|+word|+sex|f')
	...
	it ('supports search by age', () => {
		cy.visit('http://localhost:8984/vicav/#map=[biblMarkers,_samples_,]&1=[crossSamplesResult,location|Test+age|0%2c30+person|+sentences|+word|+sex|')
		cy.get('[data-snippetID=compare-samples-result]').then(() => {
